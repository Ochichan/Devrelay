...
| M9 Environment hydration | Partial | Trust hashes, bootstrap fingerprint file content hashing, profile selection, Nix, Dev Container, native bootstrap adapter, secret provider materialization paths, and the core hydration state machine with JSON persistence and `environment.progress` payloads exist. Environment doctor and CLI/UI visibility remain open. |
