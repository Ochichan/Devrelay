...
| M13 Security and operations | Partial | Threat model, fuzzing, diagnostics, fault injection, secret scanning pieces, false-positive override design, encrypted one-time sidecar design, release/update policy, backup anchor data set, opaque anchor research decision, and signed backup anchor replication with verify/restore and manual audited promotion exist. Independent review remains open. |
