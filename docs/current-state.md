...
| M9 Environment hydration | Partial | Trust hashes, bootstrap fingerprint file content hashing, profile selection, Nix, Dev Container, native bootstrap adapter, secret provider materialization paths, the core hydration state machine with JSON persistence and `environment.progress` payloads, `environment.status`, `devrelay doctor environment`, `devrelay environment status`, and desktop Continue hydration visibility exist. Environment failure integrity and representative shell-ready SLO evidence remain open. |
