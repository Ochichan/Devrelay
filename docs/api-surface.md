...
`projects.list`, `workspaces.list`, and `sessions.snapshots.list`, but the
project still needs a remote RPC server over mTLS, remaining method handlers,
and integration tests around that server.
